...
async function main(): Promise<void> {
  ...
  const integrations = sdk.integrations as {
    listWebhooks?: (p: { project_id: number }) => Promise<{ data?: unknown[] }>;
  };
  const hooks = await integrations.listWebhooks?.({ project_id: projectId });
