...
function parseArgs(argv) {
  let target = null;
  let dryRun = false;
  let removeSubmodule = false;
  for (let i = 2; i < argv.length; i++) {
    if (argv[i] === '--target') target = argv[++i];
    else if (argv[i] === '--dry-run') dryRun = true;
    else if (argv[i] === '--remove-submodule') removeSubmodule = true;
  }
  if (!target) {
    console.error(
      'Usage: node uninstall.mjs --target <path> [--dry-run] [--remove-submodule]',
    );
    process.exit(1);
  }
  return { target: path.resolve(target), dryRun, removeSubmodule };
}
...
function deinitKitSubmodule(target, subPath, dryRun) {
  if (dryRun) {
    console.log('[dry-run] git submodule deinit -f', subPath);
    return;
  }
  spawnSync('git', ['submodule', 'deinit', '-f', subPath], { cwd: target, stdio: 'inherit' });
  const abs = path.join(target, subPath);
  if (fs.existsSync(abs)) fs.rmSync(abs, { recursive: true, force: true });
}

function main() {
  const { target, dryRun, removeSubmodule } = parseArgs(process.argv);
  ...
  if (removeSubmodule) {
    ...
    deinitKitSubmodule(target, subPath, dryRun);
    console.warn('Removed git submodule at', subPath);
  } else {
    console.log('Kit submodule left in place (use --remove-submodule to deinit)');
  }
